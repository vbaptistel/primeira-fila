...
| `TASK-026` | `US-012` | Frontend Backoffice | Integrar Supabase Auth para login e sessão no `web-backoffice`. | Login funcional via provider e sessão segura persistida com cookie `HttpOnly`. | `TASK-015` | `TODO` |
| `TASK-027` | `US-012` | Frontend Backoffice | Implementar logout e revogação de sessão no Supabase Auth no `web-backoffice`. | Sessão removida no cliente e token inválido para novas chamadas protegidas. | `TASK-026` | `TODO` |
...
| `TASK-064` | Doc 18 | Backend | Implementar serviço Supabase Admin para criar/atualizar usuários com app_metadata (role, tenant_id). | Serviço encapsula Supabase Admin API; valida role e existência do tenant; usado apenas por controllers protegidos. | `TASK-015` | `TODO` |
| `TASK-065` | Doc 18 | Backend | Adicionar campo Tenant.maxUsers, config default (env) e tabela tenant_members. | Migration e modelo; contagem e listagem de usuários do tenant via tenant_members; limite aplicado na criação. | `TASK-061` | `TODO` |
| `TASK-066` | Doc 18 | Backend | Implementar endpoints POST/GET /v1/tenants/:tenantId/users (e opcional PATCH), validação de limite e RBAC. | platform_admin ou organizer_admin (próprio tenant); criar usuário sincroniza tenant_members; códigos USER_LIMIT_EXCEEDED, USER_ALREADY_EXISTS. | `TASK-064`,`TASK-065` | `TODO` |
| `TASK-067` | Doc 18 | Backend | Implementar GET /v1/tenants (listagem paginada para platform_admin). | Listar todos os tenants; apenas platform_admin. | `TASK-015` | `TODO` |
| `TASK-068` | Doc 18 | Frontend Backoffice | Criar tela "Usuários" no dashboard do tenant (listar e adicionar usuário). | Consome GET/POST /v1/tenants/:tenantId/users; organizer_admin e platform_admin (ao acessar o tenant). | `TASK-026`,`TASK-066` | `TODO` |
| `TASK-069` | Doc 18 | Frontend Plataforma | Criar app web-platform para platform_admin (lista de tenants, criar tenant, criar/listar usuários). **Adiado para fase posterior.** | App separada; sessão platform_admin; gestão completa de onboarding. Não incluída no MVP atual. | `TASK-066`,`TASK-067` | `TODO` |
...
| `6` | Backoffice Base | `Sprint 06` | Entregar gestão operacional inicial no `web-backoffice` e autenticação (login/logout) para operadores. | `TASK-026`,`TASK-027`,`TASK-028`,`TASK-029`,`TASK-030` |
...
| `9` | Onboarding e usuários | `Sprint 09` | Backend: gestão de usuários por tenant (serviço Supabase Admin, tenant_members, maxUsers, endpoints). GET /v1/tenants. Backoffice: tela Usuários do tenant. Frontend plataforma (web-platform) adiado. | `TASK-064`,`TASK-065`,`TASK-066`,`TASK-067`,`TASK-068` |
...
## Changelog
- `v1.25.0` - 2026-02-15 - Onboarding de tenant e gestão de usuários (Documento 18). Novas tasks: TASK-064 a TASK-069 (serviço Supabase Admin, tenant_members, maxUsers, endpoints POST/GET /v1/tenants/:tenantId/users e GET /v1/tenants, tela Usuários no backoffice). Frontend de plataforma (web-platform) adiado para fase posterior; Sprint 09 "Onboarding e usuários" adicionada. Documentação transversal atualizada (docs 03, 05, 06, 18, README).
- `v1.24.3` - 2026-02-15 - Provisionamento automático de subdomínio na Vercel ao criar tenant: `SubdomainProvisioningService` adiciona `{subdomain}.{baseDomain}` ao projeto Vercel do web-customer e, quando o domínio base está no Vercel DNS, cria o registro CNAME via API. Cliente compartilhado `VercelApiClient` extraído para `CustomDomainService` e `SubdomainProvisioningService`. Variáveis `PLATFORM_SUBDOMAIN_BASE_DOMAIN` (opcional) e reutilização de `VERCEL_*`. Falhas de provisionamento não impedem a criação do tenant (apenas log). Testes unitários: 8 para `SubdomainProvisioningService`, integração em `createTenant` e refatoração dos testes de `CustomDomainService`. Documentação em `docs/14-deploy-backend-vercel.md`.
- `v1.24.2` - 2026-02-15 - TASK-026 e TASK-027: escopo restrito ao `web-backoffice` (Supabase Auth para operadores/organizadores); área alterada de "Frontend" para "Frontend Backoffice"; tasks incluídas na Sprint 06 (Backoffice Base). O web-customer segue com acesso via magic link sem login.
- `v1.24.1` - 2026-02-15 - Melhoria de branding white-label: remoção do conflito com `prefers-color-scheme` em `globals.css` do web-customer; `applyTenantBranding` passa a definir `--background`, `--foreground` e paleta derivada (surface, text, muted-text, border) com base em `colorScheme`; novo parâmetro `colorScheme` ("light"|"dark"|"system") no model `Tenant`, DTOs, service e `getPublicBranding`; frontend aplica paleta conforme `colorScheme` e escuta mudanças em `prefers-color-scheme` quando `colorScheme === "system"`; seed atualizado (Arena Shows com `colorScheme: "dark"` e `secondaryColor` escura).
